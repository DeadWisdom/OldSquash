...
class DictSchema(object):
    """
        ...
        4
    """
    def __init__(self, structure, all_optional=False, all_none=False):
        self.structure = structure
        for k, v in structure.items():
            if all_optional:
                self.structure[k] = Any(v, nil)
            elif all_none:
                self.structure[k] = Any(v, or_none)
            else:
                self.structure[k] = Schema(v)
    
    def __call__(self, value):
        build = {}
        for k, type_ in self.structure.items():
            field = k
